...
class ECAPA_TDNN(torch.nn.Module):
    ...
    def forward(self, x, lengths=None):
        ...
        x_t = self.fc_t(x_t).transpose(1, 2)
...
class Classifier(torch.nn.Module):
    ...
    def __init__(
        ...
        nn.init.xavier_uniform_(self.weight_t)

        # self.weight_t_ori = nn.Parameter(
        #     torch.FloatTensor(192, input_size, device=device)
        # )
        # nn.init.xavier_uniform_(self.weight_t_ori)
